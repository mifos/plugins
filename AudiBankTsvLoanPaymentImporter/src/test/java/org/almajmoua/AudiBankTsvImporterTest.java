...
public class AudiBankTsvImporterTest {
    TransactionImport transactionImport;
    AudiBankTsvImporter concreteImporter;
    @Mock
    ...
    public void setUpBeforeMethod() throws Exception {
        concreteImporter = new AudiBankTsvImporter();
    ...
    public void successfulImport() throws Exception {
        String testDataFilename = this.getClass().getResource("/audi_test.txt").getFile();
    ...
    public void successfulImportWithMifosId() throws Exception {
        String testDataFilename = this.getClass().getResource("/audi_test_mifos_id.txt").getFile();
        ParseResultDto result = transactionImport.parse(new FileInputStream(testDataFilename));
        assertThat(result.getParseErrors().toString(), result.getParseErrors().size(), is(0));
    ...
    /**
     * Little-endian UTF-16 is the file encoding used for Audi imports.
     */
    @Test
    public void successfulImportOfUTF16LittleEndian() throws Exception {
        String testDataFilename = this.getClass().getResource("/audi_test_UTF-16LE.txt").getFile();
        ParseResultDto result = transactionImport.parse(new FileInputStream(testDataFilename));
        assertThat(result.getParseErrors().toString(), result.getParseErrors().size(), is(0));
    }
    ...
    public void missingData() {
        InputStream input = new ByteArrayInputStream("blah".getBytes());
        ParseResultDto result = transactionImport.parse(input);
        assertThat(result.getParseErrors().toString(), result.getParseErrors().size(), is(1));
        assertThat(result.getSuccessfullyParsedRows().toString(), result.getSuccessfullyParsedRows().size(), is(0));
    }

    @Test
    public void badDateFormat() throws Exception {
        String testDataFilename = this.getClass().getResource("/audi_test_bad_date_format.txt").getFile();
        ParseResultDto result = transactionImport.parse(new FileInputStream(testDataFilename));
        assertThat(result.getParseErrors().toString(), result.getParseErrors().size(), is(2));
        assertThat(result.getParseErrors().toString(), result.getParseErrors().get(0),
                containsString("does not follow expected format"));
        assertThat(result.getParseErrors().toString(), result.getParseErrors().get(1),
                containsString("does not follow expected format"));
    }

    @Test
    public void missingSerialNumber() throws Exception {
        String testDataFilename = this.getClass().getResource("/audi_test_bad_serial_number.txt").getFile();
        ParseResultDto result = transactionImport.parse(new FileInputStream(testDataFilename));
        assertThat(result.getParseErrors().toString(), result.getParseErrors().size(), is(1));
